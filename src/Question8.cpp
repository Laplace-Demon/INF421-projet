...
*/
    vector<vector<int>> maximalRate(instance.channelNumber, vector<int>(instance.totalEnergy + 1, 0));
    for (int i = 1, pos = instance.feasibleChoices[0][0].cost; i < instance.feasibleChoices[0].size(); ++i) {
        for (; pos <= instance.feasibleChoices[0][i].cost; ++pos) maximalRate[0][pos] = instance.feasibleChoices[0][i - 1].rate;
    }
    for (int i = 1; i < instance.channelNumber; ++i)
        for (int j = 1; j <= instance.totalEnergy; ++j)
            for (Choice temp : instance.feasibleChoices[i])
                maximalRate[i][j] = max(maximalRate[i - 1][j - temp.cost] + temp.rate, maximalRate[i][j]);
    ...
    for (int i = instance.channelNumber - 1, &j = instance.totalEnergy; i > 0; --i) {
        for (Choice temp : instance.feasibleChoices[i]) {
            if (maximalRate[i][j] == maximalRate[i - 1][j - temp.cost] + temp.rate) {
